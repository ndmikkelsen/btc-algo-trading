...
if [ -n "$CONSTITUTION_CHANGED" ]; then
  ...
  for file in CONSTITUTION.md VISION.md PLAN.md; do
    curl -X POST https://btc-cognee.apps.compute.lan/api/v1/add \
      -F "data=@${file}" \
      -F "datasetName=btc-constitution"
  done
  
  # Cognify
  curl -X POST https://btc-cognee.apps.compute.lan/api/v1/cognify \
...
curl -X GET https://btc-cognee.apps.compute.lan/api/v1/datasets | grep constitution
...
- Verify dataset exists: `curl https://btc-cognee.apps.compute.lan/api/v1/datasets`
